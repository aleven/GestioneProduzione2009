...
Public Class frmAnagraficaFornitori
...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(frmAnagraficaFornitori))
        ...
        Me.lblCodFornitore.Text = "Codice Fornitore :"
        '
        'txtCodFornitore
        '
        ...
        Me.btnEliminaFornitore.TabIndex = 21
        Me.ToolTip1.SetToolTip(Me.btnEliminaFornitore, "Elimina fornitore")
        '
        'btnNuovoFornitore
        '
        ...
        Me.btnNuovoFornitore.TabIndex = 20
        Me.ToolTip1.SetToolTip(Me.btnNuovoFornitore, "Nuovo fornitore")
        '
        'btnSceltaFornitore
        '
        ...
        Me.btnSceltaFornitore.TabIndex = 19
        Me.ToolTip1.SetToolTip(Me.btnSceltaFornitore, "Ricerca e selezione fornitore")
        '
        'lblTelefono
        '
        ...
        Me.Name = "frmAnagraficaFornitori"
        Me.StartPosition = System.Windows.Forms.FormStartPosition.CenterScreen
        Me.Text = "Anagrafica Fornitori"
...
#Region "Eventi Form"
    ...
    Private Sub frmFornitori_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub frmFornitori_Closing(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles MyBase.Closing
...
#Region "Eventi Controlli Form"
    ...
    Private Sub btnSceltaFornitore_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSceltaFornitore.Click

        'clessidra
        Cursor.Current = Cursors.WaitCursor

        Dim FormSceltaFornitore As New frmSceltaFornitore
        FormSceltaFornitore.ShowDialog()

        If FormSceltaFornitore.IDFornitore <> -1 Then

            m_IDFornitore = FormSceltaFornitore.IDFornitore()
            Call CaricaDatiFornitore()
    ...
    Private Sub btnNuovoFornitore_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnNuovoFornitore.Click

        If m_Modificato Then
            If DialogResult.Yes = MessageBox.Show("Salvare le modifiche apportate prima dell'inserimento nuovo fornitore?", Me.Text, MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) Then
    ...
    Private Sub btnEliminaFornitore_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnEliminaFornitore.Click

        If Not m_Variazione Then
            MessageBox.Show("Fornitore in fase di inserimento! Impossibile eliminarlo!", Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Information)
            Exit Sub
        End If

        If m_IDFornitore = -1 Then
            MessageBox.Show("Scegliere prima un fornitore valido!", Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Information)
            Exit Sub
        End If

        If MessageBox.Show("Confermi eliminazione fornitore?", Me.Text, MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then

            Dim DRControlloGruppi As SqlClient.SqlDataReader

            'un fornitore non è eliminabile se fa parte di un' ordine non eliminato
            m_SQL = ""
            m_SQL += "SELECT * FROM t11_fornitori INNER JOIN t08_testate_ordini_for ON "
            m_SQL += "t11_id_fornitore = t08_id_fornitore_t11 "
            m_SQL += "WHERE t08_dt_cancellazione IS NULL AND "
            m_SQL += "t11_dt_cancellazione IS NULL AND "
            m_SQL += "t11_id_fornitore = " & m_IDFornitore.ToString
            ...
            m_SQL = "UPDATE t11_fornitori SET t11_dt_cancellazione = GetDate() WHERE t11_id_fornitore = " & m_IDFornitore.ToString
...
#Region "Sub e Function personalizzate"
    ...
    Private Sub CaricaDatiFornitore()
        ...
        m_SQL = "SELECT t11_fornitori.* FROM t11_fornitori WHERE t11_id_fornitore = " & m_IDFornitore.ToString
        ...
        If DRFornitore.HasRows Then
            ...
            If DRFornitore("t11_cod_fornitore").GetType Is Type.GetType("System.DBNull") Then
                Me.txtCodFornitore.Text = ""
            Else
                Me.txtCodFornitore.Text = DRFornitore("t11_cod_fornitore")
            End If

            If DRFornitore("t11_descrizione").GetType Is Type.GetType("System.DBNull") Then
                Me.txtDescrFornitore.Text = ""
            Else
                Me.txtDescrFornitore.Text = DRFornitore("t11_descrizione")
            End If

            If DRFornitore("t11_telefono").GetType Is Type.GetType("System.DBNull") Then
                Me.txtTelefono.Text = 0
            Else
                Me.txtTelefono.Text = DRFornitore("t11_telefono")
            End If

            If DRFornitore("t11_note").GetType Is Type.GetType("System.DBNull") Then
                Me.txtNote.Text = 0
            Else
                Me.txtNote.Text = DRFornitore("t11_note")
            End If
            ...
            m_SQL = "SELECT * FROM t08_testate_ordini_for WHERE "
            m_SQL += "t08_id_fornitore_t11 = " & m_IDFornitore.ToString
            ...
            If DRIstanze.HasRows Then
                MessageBox.Show("ATTENZIONE! Il fornitore che si sta richiamando appartiene ad un ordine!", Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Information)
            End If

        Else

            MessageBox.Show("Fornitore non memorizzato!", Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Information)
    ...
    Private Function SalvaDati() As Boolean
        ...
        If m_Variazione Then    'UPDATE
            ...
            m_SQL = ""
            m_SQL += "UPDATE t11_fornitori SET "
            m_SQL += "t11_cod_fornitore = " & DoppioApostrofo(Me.txtCodFornitore.Text) & ", "
            m_SQL += "t11_descrizione = " & DoppioApostrofo(Me.txtDescrFornitore.Text) & ", "
            m_SQL += "t11_telefono = " & DoppioApostrofo(Me.txtTelefono.Text) & ", "
            m_SQL += "t11_note = " & DoppioApostrofo(Me.txtNote.Text) & ", "
            m_SQL += "t11_dt_modifica = GetDate() WHERE "
            m_SQL += "t11_id_fornitore = " & m_IDFornitore.ToString

        Else    'insert

            m_SQL = ""
            m_SQL += "INSERT INTO t11_fornitori (t11_cod_fornitore, t11_descrizione, "
            m_SQL += "t11_telefono, t11_note) VALUES ("
    ...
    Private Function CodiceFornitoreDoppio() As Boolean
        ...
        'l'archivio.
        m_SQL = "SELECT * FROM t11_fornitori WHERE t11_dt_cancellazione IS NULL AND "
        m_SQL += "t11_cod_fornitore = " & DoppioApostrofo(Me.txtCodFornitore.Text)
        If m_Variazione Then
            m_SQL += " AND t11_id_fornitore <> " & m_IDFornitore.ToString
        End If
